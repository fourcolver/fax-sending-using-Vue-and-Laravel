...
//
        ...
        $pdf->Cell(40, 85, Carbon::now('Europe/Amsterdam')->format('d-m-Y, H:i') . " uur");

        $pdf->Ln(10);
        $pdf->Cell(35, 90, 'Aanvraag:');
        $pdf->Cell($pdf->GetStringWidth($fax->applied_for . "  "), 90, $fax->applied_for);
        $pdf->Cell($pdf->GetStringWidth('(kenmerk: '. $fax->subject. ")"), 90, '(kenmerk: '. $fax->subject. ")");
//        if($fax->subject != null) {
//            $pdf->Cell($pdf->GetStringWidth($fax->subject . ") "), 90, $fax->subject. " )");
//        }

        $pdf->Ln(3);
        $pdf->Cell(35, 95, 'Datum aanvraag:');
        $pdf->Cell(40, 95, date('d-m-Y', strtotime($fax->date)));
...
//body goes here
        ...
        $pdf->Write(5, "Op " .  date('d-m-Y', strtotime($fax->date)) . " heeft" ." ". $customer->gender ." ". $customer->first_name ." ". $customer->last_name . " het volgende aangevraagd: " . $fax->applied_for ." ". $customer->gender ." ".$customer->last_name . " heeft geen beslissing ontvangen.
...
//add date
        ...
        $pdf->Ln(5);
        $pdf->Cell(35, 5, '');
        $pdf->Cell($pdf->GetStringWidth("Beslisapp.nl"), 5, "Beslisapp.nl");
...
//**************************************
        ...
        $pdf->Ln(3);
        $pdf->Cell(35, 85, 'En:');
        $pdf->Cell(100, 85, "Ralph Vincent Tjon, Beslisapp.nl");
...
//we have to break down the date here. or just change it in date picker
        ...
        $pdf->Cell($pdf->GetStringWidth(date('d-m-Y', strtotime($fax->date))), 100, date('d-m-Y', strtotime($fax->date)));

        $pdf->Ln(3);
        $pdf->Cell(35, 105, 'Inzake besluit:');
        $pdf->Cell($pdf->GetStringWidth($fax->applied_for . "  "), 105, $fax->applied_for);
        $pdf->Cell($pdf->GetStringWidth('(kenmerk: '. $fax->subject. ")"), 105, '(kenmerk: '. $fax->subject. ")");
        $pdf->Ln(3);
...
//body goes here
        $pdf->Write(5, "Hierbij machtigt u Beslisapp.nl om de $government->name in gebreke te stellen. Als de gemeente $government->name niet binnen vijftien dagen een besluit neemt op uw aanvraag of bezwaarschrift is een dwangsom verschuldigd. De hoogte van de boete is  ".EURO." 20 per dag voor de eerste 14 dagen. ".EURO." 30 per dag voor de 14 dagen daarna en ".EURO." 40 per dag voor de 14 dagen daarna. De totale dwangsom bedraagt maximaal ".EURO." 1260,-. De dwangsom wordt vastegesteld door de gemeente.

De kosten voor het in gebreke stellen bedragen 30 procent van de uit te betalen dwangsom. De beslisapp incasseert de dwangsom bij de gemeente. Zeventig procent van de ontvangen dwangsom wordt binnen twee weken na ontvangst overgemaakt op uw rekeningnummer \"".$customer->bank_account."\".
